...
import com.tamertokbaev.momari.services.BookManipulationService
...
class HomeAccountFragment : Fragment() {
    ...
    private val destinationService = ServiceBuilder.buildService(AuthService::class.java)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        fetchUserInformation(view)
    }

    private fun fetchUserInformation(view: View) {
        val preferences = view.context.getSharedPreferences(Constants.APP_SHARED_PREF_NAME, Context.MODE_PRIVATE)
        this.bearerToken = preferences.getString(Constants.APP_SHARED_USER_TOKEN_KEY, "")
        //initiate the service
        val requestCall = destinationService.fetchUserInformation(this.bearerToken!!);
        ...
        requestCall.enqueue(object : retrofit2.Callback<User> {
            override fun onResponse(
                ...
                val welcomeTextView = view.findViewById<TextView>(R.id.home_welcome_text)
                val imageView = view.findViewById<ImageView>(R.id.home_avatar)

                welcomeTextView.text = "Welcome, ${user.name}"
                val profile_photo: String? = user.profile_photo
                if (profile_photo != null) {
                    Picasso.get().load(profile_photo).into(imageView)
                }
            }
